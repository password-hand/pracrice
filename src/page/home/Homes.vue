    ...
    <div style="position:relative">
        ...
        <div class="content">
            ...
            <div class="books-box clearfix" :title-message="tit2">
                <h4>{{tit2}}</h4>
                <div class="book-list" v-for="book in recomend" :key="book.id" @click="bookDetails(book)">
                    <img :src="book.cover">
                    <p>{{book.name}}</p>
                    <p class="color-txt">
                        <span>￥</span> {{book.price}}</p>
            ...
            <div class="books-box clearfix" :title-message="tit3">
                <h4>{{tit3}}</h4>
                <div class="book-list" v-for="book in hotBooks" :key="book.id" @click="bookDetails(book)">
                    <img :src="book.cover">
                    <p>{{book.name}}</p>
                    <p class="color-txt">
                        <span>￥</span> {{book.price}}</p>
        ...
        <transition name="fade">
            <div v-show="change" class="booksList">
                ...
                <div class="btn">
                    <div class="btn-box">
                        <div  
                        @click="$emit('goShopping',bookList,$refs.sel.value)"> 加入购物车 </div>
                        <div  @click="showMes">立即购买</div>
                    </div>
                </div>
        </div> 
        </transition>
